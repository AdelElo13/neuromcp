...
describe('memory proxy names', () => {
  ...
  it('current derivation embeds the memory id so one proxy represents exactly one memory', () => {
    ...
    expect(a).not.toBe(b);
    expect(a).toBe('memory:Weekly maintenance summary for the Atlas platform team week #aaaaaaaa');
